...
curl -s https://raw.githubusercontent.com/aHejazi/docker-magento-237/master/lib/onelinesetup | bash -s -- bayn.test 2.3.7-p3 community
```

The `bayn.test` above defines the hostname to use, and the `2.3.7-p3` defines the Magento version to install. Note that since we need a write to `/etc/hosts` for DNS resolution, you will be prompted for your system password during setup.

After the one-liner above completes running, you should be able to access your site at `https://bayn.test`.
...
Same result as the one-liner above. Just replace `bayn.test` references with the hostname that you wish to use.
...
- `bin/setup`: Run the Magento setup process to install Magento from the source code, with optional domain name. Defaults to `bayn.test`. Ex. `bin/setup bayn.test`
- `bin/setup-composer-auth`: Setup authentication credentials for Composer.
- `bin/setup-domain`: Setup Magento domain name. Ex: `bin/setup-domain bayn.test`
- `bin/setup-grunt`: Install and configure Grunt JavaScript task runner to compile .less files
- `bin/setup-pwa-studio`: (BETA) Install PWA Studio (requires NodeJS and Yarn to be installed on the host machine). Pass in your base site domain, otherwise the default `master-7rqtwti-mfwmkrjfqvbjk.us-4.magentosite.cloud` will be used. Ex: `bin/setup-pwa-studio bayn.test`
- `bin/setup-ssl`: Generate an SSL certificate for one or more domains. Ex. `bin/setup-ssl bayn.test foo.test`
...
5. Open `PhpStorm > Preferences > PHP > Servers` and create a new server:
    ...
    * For the Name, set this to the value of your domain name (ex. `bayn.test`).
    * For the Host, set this to the value of your domain name (ex. `bayn.test`).
    * Keep port set to `80`.
...
6. Go to `Run > Edit Configurations` and create a new `PHP Remote Debug` configuration.
    ...
    * Set the Name to the name of your domain (ex. `bayn.test`).
...
7. Open up `pub/index.php` and set a breakpoint near the end of the file.
    ...
    * Start the debugger with `Run > Debug 'bayn.test'`, then open up a web browser.
